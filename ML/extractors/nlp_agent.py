...
URGENCY_KEYWORDS = [
    "hurry", "quick", "fast", "now", "today only", "limited time",
    "don't miss", "act now", "expires", "last chance", "ending soon",
    "running out", "final", "deadline", "before it's gone", "while stocks last",
...
class NLPAgent:
    ...
    def _detect_urgency(self, element: UIElement, text: str) -> list[NLPFeature]:
        """Detect false urgency signals in text."""
    ...
    def _detect_emotional_manipulation(self, element: UIElement, text: str) -> list[NLPFeature]:
        """Detect emotional manipulation tactics (guilt, fear, sadness)."""
        text_lower = text.lower()
    ...
    def _detect_sentiment_asymmetry(self, elements: list[UIElement]) -> list[NLPFeature]:
        ...
        positive_words = {"yes", "accept", "agree", "continue", "ok", "great", "love",
                         "best", "buy", "keep", "get", "start", "join", "subscribe", "🎉", "🎁"}
        negative_words = {"no", "cancel", "decline", "reject", "skip", "later", "close",
                         "don't", "not", "stop", "quit", "leave", "unsubscribe", "exit"}
