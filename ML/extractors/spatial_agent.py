...
class SpatialAgent:
    ...
    def _check_size_asymmetry(self, elements: list[UIElement]) -> list[SpatialFeature]:
        ...
        positive_keywords = {"accept", "agree", "yes", "ok", "continue", "buy", "subscribe",
                           "keep", "confirm", "start", "join", "get", "add"}
        negative_keywords = {"reject", "decline", "no", "cancel", "skip", "close", "later",
                           "exit", "leave", "unsubscribe", "manage", "preferences", "dismiss"}
        ...
        # Compare sizes of positive vs negative actions
        for pos in positive_elements:
            for neg in negative_elements:
                ...
                if area_ratio >= SIZE_ASYMMETRY_RATIO:
                    severity = "critical" if area_ratio >= 10 else ("high" if area_ratio >= 5 else "medium")
                    features.append(SpatialFeature(
                        element_id=neg.id,
    ...
    def _check_visual_hierarchy(self, elements: list[UIElement]) -> list[SpatialFeature]:
        ...
        - Positive action should not have z-index layered above negative action
        """
        features = []
        ...
        for element in interactive:
            ...
            # Check if decline/cancel buttons are deliberately made less visible
            text_lower = element.text.lower()
            is_negative_action = any(kw in text_lower for kw in [
                "cancel", "no", "decline", "skip", "reject", "close",
                "manage", "preferences", "don't", "not"
            ])
    ...
    def _check_reading_flow(self, elements: list[UIElement]) -> list[SpatialFeature]:
        ...
        important_keywords = ["fee", "charge", "cost", "price", "terms", "conditions",
                            "recurring", "subscription", "auto-renew", "cancel"]
            ...
            if matched_keywords and element.bounding_box:
                ...
                if page_elements_with_bbox:
                    ...
                    if max_y > 0:
                        ...
                        # Important info at the bottom 10% + small/faded text = suspicious
                        is_bottom = vertical_position_ratio > 0.85
                        is_small = False
                        is_faded = False
                        ...
                        if element.computed_css:
                            ...
                            if element.computed_css.opacity:
                                try:
                                    is_faded = float(element.computed_css.opacity) < 0.6
                                except ValueError:
                                    pass

                        if is_bottom and (is_small or is_faded):
                            features.append(SpatialFeature(
                                element_id=element.id,
