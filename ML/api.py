...
class RAGQueryRequest(BaseModel):
    question: str = Field(..., description="The user's compliance question")
    audit_context: Optional[str] = Field(
        None,
        description="JSON string of the user's latest audit report for contextual answers",
    )
    chat_history: Optional[list[dict]] = Field(
        None,
        description="List of {role, content} messages from the conversation",
    )
...
@app.post("/api/v1/rag/query", response_model=RAGQueryResponse, tags=["RAG Agent"])
async def rag_query(
    request: RAGQueryRequest,
):
    """
    **Compliance Q&A** — ask questions about DPDP Act, EU AI Act,
    Consumer Protection Act, and your website audit results.

    No API key required — this endpoint is called from the authenticated
    Node.js backend which handles user auth.
    """
    try:
...
@app.post("/api/v1/rag/initialize", response_model=RAGInitResponse, tags=["RAG Agent"])
async def rag_initialize():
    """
    **Initialize RAG Engine** — pre-load and embed the knowledge base.
    Call this once on startup to warm up the engine.
    """
    try:
        engine = get_rag_engine()
        engine.initialize()
        return RAGInitResponse(
            status="ready",
            chunks_loaded=len(engine._store),
            message=f"RAG engine initialized with {len(engine._store)} knowledge chunks",
        )
    except Exception as e:
        logger.error(f"RAG initialization failed: {e}\n{traceback.format_exc()}")
        raise HTTPException(status_code=500, detail=f"RAG init error: {str(e)}")
